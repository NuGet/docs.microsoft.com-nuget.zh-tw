...
如果使用, `packages.config`通常位於專案根目錄中。 當第一個 NuGet 作業執行時, 它會自動建立, 但也可以手動建立, 然後再執行任何命令`nuget restore`, 例如。
...
| allowedVersions | 否 | 此套件在套件更新期間套用的允許版本範圍 (請參閱[限制升級版本](../consume-packages/reinstalling-and-updating-packages.md#constraining-upgrade-versions)。 它「不」會影響在安裝或還原作業期間所安裝的套件。 如需語法，請參閱[套件版本控制](../concepts/package-versioning.md#version-ranges-and-wildcards)。 PackageManager UI 也會停用允許範圍之外的所有版本。 | 
