...
| build | MSBuild `.targets` 和 `.props` 檔案 | 自動插入至專案檔 (NuGet 2.x) 或 `project.lock.json` (NuGet 3.x+)。 |
...
NuGet 2.x 使用 `\build` 檔案安裝套件時，會在指向 `.targets` 和 `.props` 檔案的專案檔中新增 MSBuild `<Import>` 項目  (`.props` 新增於專案檔頂端；`.targets` 則新增於底端)。
