...
*針對使用 NuGet 4.0+ 的 .NET Core 專案，請參閱 [NuGet 封裝和還原為 MSBuild 目標](../schema/msbuild-targets.md)，以取得跨目標的詳細資料。*
...
- [架構版本資料夾結構](#framework-version-folder-structure)
- [比對組件版本與專案中的目標架構](#matching-assembly-versions-and-the-target-framework-in-a-project)
- [依架構版本來群組組件](#grouping-assemblies-by-framework-version)
- [判斷要使用的 NuGet 目標](#determining-which-nuget-target-to-use)
- [內容檔案和 PowerShell 指令碼](#content-files-and-powershell-scripts)
...
如需所支援名稱的完整清單，請參閱[目標架構參考](../schema/target-frameworks.md#supported-frameworks)。
...
> [!Warning]
> 只有使用 `packages.config` 格式才能使用可變動的內容檔案和指令碼執行；可變動的內容檔案和指令碼執行已在使用 `project.json` 和 PackagesReference 格式時遭取代，而且不應該用於任何新套件。
