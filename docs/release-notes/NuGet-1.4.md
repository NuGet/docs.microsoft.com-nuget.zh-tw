...
`-Safe` 旗標會限制只能升級至具有相同主要和次要版本元件的版本。 例如，如果已安裝版本1.0.0 的封裝，而摘要中有版本1.0.1、1.0.2 和1.1，則 `-Safe` 旗標會將套件更新為1.0.2。 升級時若沒有 `-Safe` 旗標，會將套件升級至最新版本1.1。
...
根據預設，在封裝上執行 `Update-Package` 命令時（或使用對話方塊來更新套件），它會更新為摘要中的最新版本。 有了更新所有封裝的新支援，您可能會想要將封裝鎖定至特定版本範圍。 例如，您可能事先知道，您的應用程式只會使用版本 2. * 的套件，而不是3.0 和更新版本。 為了避免不小心將套件更新為3，NuGet 1.4 新增了使用新的 `allowedVersions` 屬性手動編輯 `packages.config` 檔案，以限制封裝可升級為之版本的支援。

例如，下列範例示範如何鎖定 `SomePackage` 封裝版本範圍 2.0-3.0 （獨佔）。
`allowedVersions` 屬性會接受使用[版本範圍格式](../concepts/package-versioning.md#version-ranges)的值。
...
透過 **工具**  -> 連結**庫套件管理員** -> **封裝視覺化檢視** 功能表選項所啟動的新封裝視覺化檢視，可讓您輕鬆地將方案中的所有專案及其套件相依性視覺化。
...
某些版本的 NuGet 會引進透過 `.nuspec` 檔案表示的新功能，舊版 NuGet 對話不會加以瞭解。
...
這項功能實際上是在 NuGet 1.3 中新增，但是在更新至1.3 （例如 NuGet 1.4）之前，並不會顯示。

### <a name="package-manager-dialog-improvements"></a>套件管理員對話方塊改善
* **改良的功能表名稱**：為了清楚起見，啟動對話方塊的功能表選項已重新命名。 [功能表] 選項現在是 [**管理 NuGet 封裝**]。
* [**詳細資料] 窗格會顯示最新的更新日期**：當選取 [**線上**] 或 [**更新**] 索引標籤時，[NuGet] 對話方塊會在套件的詳細資料窗格中顯示最新的更新日期。
...
* **提示支援**：套件管理員主控台現在支援透過 `$host.ui.Prompt` 和 `$host.ui.PromptForChoice` 命令提示。
* **封裝來源名稱**：使用 `-Source` 旗標指定封裝來源時，支援提供封裝來源的名稱。

### <a name="nugetexe-command-line-improvements"></a>nuget.exe 命令列改良功能
* **Nuget 自訂命令**： nuget.exe 可透過使用 MEF 的自訂命令進行擴充。
* **簡化建立符號套件的工作流程**： `-Symbols` 旗標可以套用至一般慣例式資料夾結構，只需在資料夾內包含來源和 `.pdb` 檔案，即可建立符號套件。
* **指定多個來源**： `NuGet install` 命令支援使用分號做為分隔符號來指定多個來源，或多次指定 `-Source`。
* **Proxy 驗證支援**： nuget 1.4 新增在需要驗證的 Proxy 後方使用 NuGet 時，提示輸入使用者認證的支援。
* **Nuget.exe 更新中斷性變更**：現在必須要有 `-Self` 旗標，nuget.exe 才會自行更新。 `nuget.exe Update` 現在會採用 `packages.config` 檔案的路徑，並嘗試更新套件。 請注意，這項更新的限制在於，它不會： * * 更新、新增、移除專案檔中的內容。
...
最新版本的 `NuGet.Server` 會加入名為 `apiKey`的新 `appSetting`。 當省略金鑰或保留空白時，會停用將套件推送至摘要的功能。 將 apiKey 設定為值（最好是強式密碼），可以使用 nuget.exe 來推送套件。
...
* [問題 603](http://nuget.codeplex.com/workitem/603)：指定特定封裝來源時，跨不同存放庫的套件相依性解析正確。
* [問題 1036](http://nuget.codeplex.com/workitem/1036)：將 `NuGet Pack SomeProject.csproj` 新增至後置組建事件不會再造成無限迴圈。
...
### <a name="bug-fixes"></a>錯誤修正
* [問題 1220](http://nuget.codeplex.com/workitem/1220)：當有一個以上的專案時，更新套件不會在所有專案中執行 `install.ps1`/`uninstall.ps1`
