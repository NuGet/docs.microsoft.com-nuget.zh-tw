...
通常,我們不支援刪除發佈到NuGet.org的包。瞭解更多關於我們[有關刪除套件的政策](policies/deleting-packages.md)。

**是否可以為預計在未來發行的套件保留名稱？**

是。 您可以通過為帳戶請求包 ID 前置,在[NuGet.org](https://www.nuget.org/)上為包預留 ID。 若要要求套件識別碼首碼，請遵循[文件](id-prefix-reservation.md)中的指示。

**如何宣告套件擁有權？**

請參考[管理NuGet.org 套件擁有者](../nuget-org/publish-a-package.md#managing-package-owners-on-nugetorg)。

**如何處理違反我軟體授權的套件擁有者？**

鼓勵 NuGet 社群合作，以解決套件擁有者與其他軟體擁有者之間可能發生的任何爭議。 我們已製作可在要求 NuGet.org 管理員仲裁之前遵循的[爭議解決程序](policies/dispute-resolution.md)。

**是否建議將我的測試包上載到NuGet.org?**
...
- 下載[溫地鐵](https://sourceforge.net/projects/winmtr/files/WinMTR-v092.zip/download)。
...
請注意[,NuGet.org密碼登錄已停止](https://blog.nuget.org/20180515/NuGet.org-will-only-support-MSA-AAD-starting-June.html),登錄NuGet.org的唯一方法是使用個人 Microsoft 帳戶 (MSA) 或 Azure 活動目錄 (AAD) 帳戶。 不過，如果您無法存取已建立關聯的 MSA/AAD 帳戶，就必須使用密碼登入來復原 NuGet.org 帳戶。 在此情況下，請遵循下列步驟。
- **要求:** 您需要有權訪問與需要恢復密碼的帳戶關聯的電子郵件。
- 前往[忘記密碼頁面](https://www.nuget.org/account/ForgotPassword)
- 輸入與要回復的NuGet.org帳戶關聯的**電子郵件地址**。
- 按一下 [傳送]**** 按鈕。
- 您會收到寄給指定電子郵件地址帳戶，且附有重設密碼連結的電子郵件。 按一下此連結，然後設定新密碼。 若您找不到電子郵件，請檢查「垃圾郵件」資料夾。
- 完成後，您便可在 NuGet 上以使用者名稱/密碼登入。
- 要使用使用者名稱/密碼登入,請使用[NuGet.org 登入頁面上](https://www.nuget.org/users/account/LogOn)**的 Nuget.org 帳戶連結登入**。
...
1. 跳到[NuGet.org 登錄頁面](https://www.nuget.org/users/account/LogOn),然後按一下「**需要幫助登入?"** 連結。
...
1. 系統會將您重新導向至 Microsoft 登入頁面。 請使用您希望將關聯更改為的帳戶登錄。 `account2@outlook.com`**注意**:您可能需要在登錄流期間按下 **「登出」並使用不同帳戶登錄**,以便能夠使用不同的 Microsoft 帳戶登錄。
1. 如果您看到如下錯誤,請參閱 Microsoft[帳戶連結到另一個NuGet.org帳戶](#microsoft-account-is-linked-with-another-nugetorg-account)瞭解更多詳細資訊。
    >_無法使用「帳戶 2」<account2@outlook.com>更新 Microsoft 帳戶。如果已連結到另一個 NuGet 帳戶,則可能發生此情況。有關詳細資訊,請聯繫支持人員。_
...
**我忘了我還有一個NuGet.org帳戶,我如何找出哪個NuGet.org帳戶?**

登錄頁面上的第二個 Microsoft[帳戶。](https://www.nuget.org/users/account/LogOn?returnUrl=%2F# "登入頁面") 這會將您登入目前與第二個 Microsoft 帳戶建立關聯的 NuGet.org 帳戶。 您接著可以檢視此帳戶上的已更新套件，及進行帳戶管理。

**我不在乎第二個NuGet.org帳戶,我想更改我的登錄第一NuGet.org帳戶與第二個 Microsoft 帳戶。我該怎麼辦?**
...
1. 按照步驟[刪除](#how-to-delete-my-nugetorg-account)第二個NuGet.org`MyNuGetAccount2`帳戶的使用者。 
1. 刪除此帳戶後，即可重試這些步驟來[變更 Microsoft 帳戶登入](#how-to-change-the-microsoft-account-i-use-for-nugetorg-login)。

**等等,我也關心第二個帳戶。我不想丟失此帳戶,但更改我的關聯帳戶登錄第一帳戶。**

您必須建立/使用第三個 Microsoft 帳戶，也就是電子郵件為 `account3@outlook.com` 的帳戶。 
1. 首先,您應該在NuGet.org`account2@outlook.com`登錄您的第二個Microsoft帳戶。按照上述步驟更改關聯的登錄名,並將第三個 Microsoft 帳戶與此NuGet.org帳戶相關聯。
...
**如何查找連結到此NuGet.org帳戶的Microsoft 帳戶?**

您應該使用[登入説明](#which-microsoft-account-is-linked-to-my-nugetorg-account)流來找出哪個 Microsoft 帳戶連結到帶`account1@outlook.com`有電子郵件位址 的NuGet.org帳戶。

**我想要使用我的 Microsoft 帳戶覆寫該帳戶**

按照["無法使用 Microsoft 登錄"中的步驟操作,如何恢復我的NuGet.org帳戶](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account)部分,將您的 Microsoft 帳戶與現有的NuGet.org帳戶相關聯。

### <a name="unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account"></a>無法使用 Microsoft 登入，如何復原我的 NuGet.org 帳戶？

如果您嘗試使用[登入説明](#which-microsoft-account-is-linked-to-my-nugetorg-account),但無法存取與您的NuGet.org帳戶關聯的Microsoft帳戶,請按照以下步驟將新的 Microsoft 帳戶連結到NuGet.org帳戶。
1. **要求**:您需要訪問與任何現有NuGet.org帳戶無關的Microsoft帳戶。 如果沒有的話，您可以[建立](https://signup.live.com)帳戶。
2. 如果您忘記 NuGet.org 帳戶的使用者名稱及密碼，請遵循[復原密碼登入的步驟](#how-to-recover-nugetorg-password-login)。
3. 使用使用者名/密碼登錄[登錄到NuGet.org。](https://www.nuget.org/users/account/LogOnNuGetAccount)
4. 登入後，就會顯示出快顯對話方塊，如下所示。 此為密碼中止對話方塊。
5. **注意**:請忽略使用指定的 Microsoft 帳戶登錄的說明。 您現可將 NuGet.org 帳戶連結到任何其他 Microsoft 登入。
...
1. **要求**:您需要先在NuGet.org創建個人帳戶,才能在組織帳戶上用作管理員。 如果沒有帳戶,請[建立新NuGet.org帳戶](individual-accounts.md)
2. 如果您沒有密碼登錄名,請按照以下步驟恢復NuGet.org帳戶的[密碼登錄](#how-to-recover-nugetorg-password-login),如果您這樣做,請跳過此步驟。
3. 使用使用者名/密碼登錄[登錄到NuGet.org。](https://www.nuget.org/users/account/LogOnNuGetAccount)
4. 登入後，就會顯示出快顯對話方塊，如下所示。 此為密碼中止對話方塊。 
    > [!Important]
...
您可以[建立](https://www.microsoft.com/account)新的 Microsoft 帳戶 (搭配**未**與 @yourdomain.com 建立關聯的電子郵件) 按照[恢復NuGet.org帳戶](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account)部分中給出的步驟操作。

### <a name="how-do-i-change-my-nugetorg-account-username"></a>如何變更我的 NuGet.org 帳戶使用者名稱？

無法變更。 作為一項政策問題,我們不允許更改使用者名。 此外,對於可能已[根據包擁有者定義包信任策略](../consume-packages/installing-signed-packages.md#trust-package-owners)的用戶來說,這樣做是一個重大更改。 唯一變更使用者名稱的方式為，以想要的使用者名稱建立新帳戶。 建議您在建立新帳戶之前，先將現有的帳戶刪除，否則您將無法重複使用註冊的 Microsoft 帳戶。
> [!Important]
...
按照[刪除NuGet.org帳戶](#how-to-delete-my-nugetorg-account)部分中給出的步驟,並註冊具有正確使用者名[的新帳戶](individual-accounts.md)。
...
請遵循下列步驟以繼續刪除帳戶。
1. 使用要刪除的帳戶[登錄NuGet.org。](https://www.nuget.org/users/account/LogOn)
2. 按下此網址[https://www.nuget.org/account/delete](https://www.nuget.org/account/delete): 然後按照步驟提交刪除帳戶的請求。
