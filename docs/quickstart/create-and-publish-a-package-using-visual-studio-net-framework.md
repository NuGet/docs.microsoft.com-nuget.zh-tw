...
> [!Note]
> 此快速入門僅適用於 Visual Studio 2017 和適用於 Windows 的更高版本。 Visual Studio for Mac 不包含這裡描述的功能。 請改為使用 [dotnet CLI 工具](create-and-publish-a-package-using-the-dotnet-cli.md)。
...
擁有`.nupkg`檔案後,將該檔發布到nuget.org`nuget.exe`使用 從nuget.org獲取的 API 金鑰。對於nuget.org您必須使用`nuget.exe`4.1.0 或更高版本。
