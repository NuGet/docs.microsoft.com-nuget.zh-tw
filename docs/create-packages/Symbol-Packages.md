...
---
# <a name="creating-legacy-symbol-packages-symbolsnupkg"></a>建立舊符號套件(.符號.nupkg)

> [!Important]
> 為符號套件新建議的格式為 .snupkg。 請參閱[建立符號套件 (.snupkg)](Symbol-Packages-snupkg.md)。 </br>
> 目前仍然支援 .symbols.nupkg，只是為達到相容性而已。

除了為nuget.org或其他源建構套件外,NuGet 還支援創建可發佈到符號伺服器的關聯符號包。 舊符號包格式 .symbols.nupkg 可以推送到 SymbolSource 儲存庫。
...
## <a name="creating-a-legacy-symbol-package"></a>建立舊符號套件

要創建舊符號包,請遵循以下約定:

- 將主要套件 (含您的程式碼) 命名為 `{identifier}.nupkg`，並包含 `.pdb` 檔案以外的所有檔案。
- 命名舊符號包`{identifier}.symbols.nupkg`,並包括程式集 DLL、`.pdb`檔、XMLDOC 檔、源檔(請參閱以下部分)。
...
## <a name="legacy-symbol-package-structure"></a>舊符號包結構

舊符號包可以像庫包一樣針對多個目標框架,因此`lib`資料夾的結構應與主包完全相同,僅`.pdb`包括 DLL 旁邊的檔。

例如,以 .NET 4.0 和 Silverlight 4 為目標的舊式符號包具有此佈局:
...
原始程式檔接著會放在名為 `src` 的個別特殊資料夾中，而此資料夾必須遵循來源存放庫的相對結構。 這是因為 PDB 包含用來編譯相符 DLL 之原始程式檔的絕對路徑，而且需要在發行程序期間找到它們。 可以剝離基本路徑(公共路徑首碼)。例如,請考慮從這些檔案產生的庫:
...
舊符號包可以由約定、從上一節中所述的資料夾結構生成,也可以通過在清單`files`部分中指定其內容來生成。 例如，若要建置上節所示的套件，請在 `.nuspec` 檔案中使用下列內容：
...
## <a name="publishing-a-legacy-symbol-package"></a>發佈舊符號包
...
2. 將主包發布到nuget.org後,按如下方式推送舊符號包,由於檔名`.symbols`中的,該包將自動使用symbolsource.org作為目標:

    ```cli
    nuget push MyPackage.symbols.nupkg
    ```

3. 要發佈到其他符號儲存庫,或推送不遵循命名約定的舊符號包,請使用以下`-Source`選項:
