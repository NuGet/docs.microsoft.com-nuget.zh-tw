...
此外，NuGet 會維護所有與套件建構方式相關的規格 (包含[當地語系化](create-packages/creating-localized-packages.md)和[偵錯符號](create-packages/symbol-packages.md)) 及其[參考](consume-packages/package-references-in-project-files.md)方式 (包含[版本範圍](concepts/package-versioning.md#version-ranges-and-wildcards)和[發行前版本](create-packages/prerelease-packages.md))。NuGet 提供各種 API 供其他應用程式以程式設計方式使用其服務，並為撰寫 Visual Studio 擴充功能和專案範本的開發人員提供支援。
