...
除此之外，NuGet 會維護套件的結構（包括[當地語系化](create-packages/creating-localized-packages.md)和[debug 符號](create-packages/symbol-packages-snupkg.md)），以及其[參考](consume-packages/package-references-in-project-files.md)方式（包括[版本範圍](concepts/package-versioning.md#version-ranges-and-wildcards)和[發行前版本）的所有相關規格。](create-packages/prerelease-packages.md).)NuGet 也提供各種 Api 來以程式設計方式處理其服務，並為撰寫 Visual Studio 延伸模組和專案範本的開發人員提供支援。
