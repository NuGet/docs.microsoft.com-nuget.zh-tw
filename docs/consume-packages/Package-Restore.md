...
- **允許 NuGet 下載遺漏的套件**會藉由變更 `packageRestore/enabled` 檔案 (在 Windows 上為 [，在 Mac/Linux 上則為 ](../reference/nuget-config-file.md#packagerestore-section)) `NuGet.Config`packageRestore 區段`%AppData%\NuGet\`的 `~/.nuget/NuGet/` 設定，控制所有形式的套件還原。 此設定也會啟用 Visual Studio 中解決方案操作功能表上的 [還原 NuGet 套件] 命令。
...
- **在 Visual Studio 建置期間自動檢查遺漏的套件**會藉由變更 `packageRestore/automatic` 檔案 [packageRestore 區段](../reference/nuget-config-file.md#packagerestore-section)中的 `NuGet.Config` 設定來控制自動還原。 將此選項設為 True 時，從 Visual Studio 中執行組建會自動還原任何遺漏的套件。 此設定不會影響從 MSBuild 命令列執行的組建。
...
> [!Important]
> 如果您直接在 `packageRestore` 中編輯 `nuget.config` 設定，請重新啟動 Visual Studio，讓 [選項] 對話方塊顯示目前的值。
...
> [!IMPORTANT]
> `restore`命令不會修改專案檔或*封裝 .config*。若要新增相依性，請在 Visual Studio 中透過套件管理員 UI 或主控台新增封裝，*或修改 package，然後*執行 `install` 或 `restore`。
...
- 使用下列其中一種方法，在還原作業之前暫時變更 *global-packages* 資料夾的位置：
  ...
  - 建立將 `NuGet.Config` (如果使用 PackageReference) 或 `globalPackagesFolder` (如果使用 `repositoryPath`) 設定為不同資料夾的 `packages.config` 檔案。 如需詳細資訊，請參閱[組態設定](../reference/nuget-config-file.md#config-section)。
  - 僅限 MSBuild：指定具有 `RestorePackagesPath` 屬性的不同資料夾。
