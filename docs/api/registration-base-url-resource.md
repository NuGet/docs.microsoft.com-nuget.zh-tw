...
項目  | 物件的陣列 | 否       | 註冊的陣列和其相關聯的中繼資料
lower  | 字串           | 是      | 頁面中最低的 SemVer 2.0.0 版本（含）
父系 (parent) | 字串           | 否       | 註冊索引的 URL
upper  | 字串           | 是      | 頁面中最高的 SemVer 2.0.0 版本（含）
...
projectUrl               | 字串                     | 否       | 
published                | 字串                     | 否       | 字串，包含發行封裝時的 ISO 8601 時間戳記
requireLicenseAcceptance | boolean                    | 否       | 
summary                  | 字串                     | 否       | 
tags                     | 字串或字串陣列  | 否       | 
title                    | 字串                     | 否       | 
version                  | 字串                     | 是      | 正規化後的完整版本字串
...
------------ | ------ | -------- | -----
id           | 字串 | 是      | 封裝相依性的識別碼
range        | 物件 (object) | 否       | 相關性的允許[版本範圍](../concepts/package-versioning.md#version-ranges)
...
------------ | ------ | -------- | -----
id           | 字串 | 是      | 替代套件的識別碼
range        | 物件 (object) | 否       | 允許的[版本範圍](../concepts/package-versioning.md#version-ranges)，或 `*` （如果允許任何版本）
