...
Name     | 在  中     | 類型    | 必要 | 注意事項
-------- | ------ | ------- | -------- | -----
LOWER_ID | {2&gt; URL&lt;2}    | string  | 是      | 封裝識別碼，小寫
...
零下  | string           | 是      | 頁面中最低的 SemVer 2.0.0 版本（含）
父 | string           | no       | 註冊索引的 URL
臨界  | string           | 是      | 頁面中最高的 SemVer 2.0.0 版本（含）
...
catalogEntry   | 物件 | 是      | 包含封裝中繼資料的目錄專案
...
range        | 物件 | no       | 相關性的允許[版本範圍](../concepts/package-versioning.md#version-ranges-and-wildcards)
...
---------------- | ---------------- | -------- | -----
描述          | 字串的陣列 | 是      | 封裝被取代的原因
訊息          | string           | no       | 此取代的其他詳細資料
alternatePackage | 物件           | no       | 應改為使用的替代套件
...
range        | 物件 | no       | 允許的[版本範圍](../concepts/package-versioning.md#version-ranges-and-wildcards)，或 `*` （如果允許任何版本）
註冊 | string | no       | 此替代封裝之註冊索引的 URL
